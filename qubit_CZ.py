...
def main():
    ...
    # ---- Create Coupler Flux Schedule ----
    coupler_flux_off = external_flux_off[1]
    coupler_flux_on  = external_flux_on[1]
    
    coupler_flux_schedule = np.full(num_steps,coupler_flux_off)
    
    # Timing indices
    start_idx = int(10e-9 / dt)  # Start ramp at 10ns
    stop_idx  = int(60e-9 / dt)  # Stop ramp at 60ns
    
    # Set the 'on' flux for the coupler
    coupler_flux_schedule[start_idx:stop_idx] = coupler_flux_on
    ...
    for i in range(n_logical):
        evolved_state, _, _, _ = solver.solve(
            ...
            coupler_flux_schedule=coupler_flux_schedule,
            time=time
        )
